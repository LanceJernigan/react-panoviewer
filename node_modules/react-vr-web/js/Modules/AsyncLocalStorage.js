 ...
 */
export default class AsyncLocalStorage extends Module {
   ...
   */
  _getRow(objectStore: IDBObjectStore, key: string): Promise<[string, ?string]> {
    return new Promise((resolve, reject) => {
   ...
   */
  multiGet(keys: Array<string>, cb: number): void {
    this._prepareDB()
      .then(db => {
      ...
      .then(
        rows => {
          this._rnctx.invokeCallback(cb, [undefined, rows]);
        },
        err => {
          this._rnctx.invokeCallback(cb, [err]);
        }
      );
  }
   ...
   * with the key that failed.
   */
  multiSet(pairs: Array<[string, string]>, cb: number): void {
    this._prepareDB()
      .then(db => {
   ...
   */
  multiRemove(keys: Array<string>, cb: number): void {
    this._prepareDB()
      .then(db => {
        const transaction = db.transaction(['pairs'], 'readwrite');
        const objectStore = transaction.objectStore('pairs');
  ...
  getAllKeys(cb: number): void {
    this._prepareDB()
      ...
      .then(
        keys => {
          this._rnctx.invokeCallback(cb, [undefined, keys]);
        },
        err => {
          this._rnctx.invokeCallback(cb, [err]);
        }
